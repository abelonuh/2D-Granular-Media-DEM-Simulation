PROGRAM particles2D
    ...
    integer, parameter :: npoints = 400000      ! Total number of time steps
    ...
    do i = 1, npoints                           ! Start integration loop
        ...
        ! 4. Resolve Wall Collisions (ELASTIC - No Damping)
        do it = 1, npar                         ! Loop over all particles
            c = -log(e)*2*(k*m(it))**0.5/3.141  ! Calculate c (unused for walls here)
            
            ! Bottom Wall (y < r) 
            if (y(it) - r(it) < 0.0d0) then
                Fy(it) = Fy(it) - k*(y(it)-r(it)) ! Force UP (Elastic)
            end if
            
            ! Top Wall (y + r > ymax) 
            if (y(it) + r(it) > ymax) then
                Fy(it) = Fy(it) - k*(y(it)+r(it)-ymax) ! Force DOWN (Elastic)
            end if
            
            ! Left Wall (x < r) 
            if (x(it) - r(it) < 0.0d0) then
                Fx(it) = Fx(it) - k*(x(it)-r(it)) ! Force RIGHT (Elastic)
            end if
            
            ! Right Wall (x + r > xmax) 
            if (x(it) + r(it) > xmax) then
                Fx(it) = Fx(it) - k*(x(it)+r(it)-xmax) ! Force LEFT (Elastic)
            end if
        end do
